...
function addBeaconFileUpload() {
	...
	if ($("#beaconFile").val() == "" && beaconNo == "") {
		alertMessage("请输入信标号或者添加信标文件。");
		return false;
	}
	...
	if ($("#beaconFile").val() != "") {
		 ...
		 * uploaded files fileElementId: the file type of input element id and
		 * it will be the index of $_FILES Array() dataType: it support json,
		 * xml secureuri:use secure protocol success: call back function when
		 * the ajax complete error: callback function when the ajax failed
		 * 
		 */
		...
		$.ajaxFileUpload({
			...
			success : function(data, status) {
				...
				try {
					...
					if (typeof (objArray[0].error) != 'undefined') {
						if (objArray[0].error != '') {
							$("#message").html(objArray[0].error);
							$('#messageDialog').modal({
								backdrop : true,
								keyboard : true,
								show : true
							...
							$("#tableBody")
									.append(
											"<tr id='"
													...
													+ obj.beaconNo
													+ "</td>"
													+ "<td><a href=\'javascript:void(0);\' onclick=\'deltr("
													+ _len + ")\'>删除</a></td>"
													+ "</tr>");
						} else {
